...
import static com.redshoes.metamodel.jdbc.JdbcDataContext.DATABASE_PRODUCT_ORACLE;
...
public class OracleQueryRewriterTest {
    ...
    public void testOffsetFetchConstruct() {
        ...
        assertEquals("There shouldn't be OFFSET-FETCH clause.", query.toSql(), qr.rewriteQuery(query));

        query.setFirstRow(offset);
        assertEquals("Wrong or missing OFFSET clause.", query.toSql() + offsetClause, qr.rewriteQuery(query));

        query.setMaxRows(rows);
        assertEquals("Wrong or missing OFFSET and FETCH clauses.", query.toSql() + offsetClause + fetchClause,
                qr.rewriteQuery(query));

        query.setFirstRow(null);
        assertEquals("Wrong or missing FETCH clause.", query.toSql() + fetchClause, qr.rewriteQuery(query));
    }
    ...
    public void testOffsetFetchVersionCheck() throws SQLException {
        ...
        assertEquals("The query shouldn't be rewritten.", query.toSql(), qr.rewriteQuery(query));
    }
